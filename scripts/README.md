...
- `dev.sh`：本地一键启动。缺依赖时 `pnpm install`；没有 `backend/.env` 时从 example 写入（不覆盖已有文件）；`DATABASE_URL` 指向 `127.0.0.1:55432` 时拉起 `db/docker-compose.yml` 的 Postgres，空库才调用 `db-reset.sh` 写入种子。随后在 3000 / 5173 空闲时启动后端和前端。Ctrl+C 只停止本次拉起的进程。
