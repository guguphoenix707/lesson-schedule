...
生产部署时设置 `NODE_ENV=production`、`DATABASE_URL`、`BETTER_AUTH_SECRET`、`BETTER_AUTH_URL` 和 `FRONTEND_ORIGIN`。Railway 同源部署时后两项使用公开 HTTPS 域名；未设置时回退 `RAILWAY_PUBLIC_DOMAIN`。服务会监听平台注入的 `PORT` 和 `0.0.0.0`。生产容器入口是 `node dist/src/main.js`；监听成功后再等待 Postgres、迁移和空库种子。只要存在 `frontend/dist` 就会静态托管，并把非 `/api` 的页面路由回退到 SPA 入口。本地开发仍保持 Vite 与 Nest 分开运行及同源代理行为。Swagger JSON 为 `GET /api/docs-json`。
