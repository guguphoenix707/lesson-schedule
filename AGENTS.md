...
本仓库按职责组织，但暂不预设编程语言、框架、组件库、数据库或部署平台。现有业务文件保持原位；除非任务明确要求，不主动搬迁或重写。
...
3. 不根据目录名猜测技术栈。新增语言、框架、组件库、数据库、构建工具或依赖前，必须有明确需求和可说明的收益。
4. 不安装依赖、不生成锁文件、不改运行环境，除非任务明确需要并获得用户授权。
