...
开发服务器默认 `http://localhost:5173`，并把 `/api` 代理到 `http://localhost:3000`，以便登录 Cookie 与页面同源。登录页 `/login`；已登录后进入 `/`。当前登录身份读 `GET /api/auth/get-session`。公开接口走 `/api/common`；业务资源走 REST 路径（如 `/api/students/:studentId`），401 表示未登录，403 表示角色不够。不调用业务 `/me`。
