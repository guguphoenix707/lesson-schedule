...
import { Alert, App, Flex, Modal, Select, Table, Typography } from 'antd';
...
import styles from './schedule-trial-modal.module.css';

type ScheduleTrialModalProps = {
  trialCaseId: string | null;
  studentName: string;
  open: boolean;
  onClose: () => void;
};
...
export function ScheduleTrialModal({
  ...
  const sessionsQuery = useQuery({
    ...
    enabled: open && Boolean(trialCaseId),
  });
  const scheduleMutation = useMutation({
    mutationFn: () => {
      if (!trialCaseId || !sessionId) {
  ...
  const dateOptions = useMemo(
    () =>
      map(uniqBy(sessions, (session) => sessionDateKey(session.startsAt)), (session) => ({
  ...
  return (
    <Modal
      ...
      okButtonProps={{ disabled: !sessionId }}
      okText="确认安排"
      open={open}
      title={`安排试听 · ${studentName}`}
      width={760}
